...
class UnitreeRos2Real(Node):
    ...
    def get_obs(self, obs_segments= None):
        ...
        for i in range(len(obs_buffer)):
            if obs_buffer[i].dim() == 1:
                # 将1维张量转为2维
                obs_buffer[i] = obs_buffer[i].unsqueeze(0)
            elif obs_buffer[i].dim() > 2:
                # 如果维度过高，将其调整为2维
                obs_buffer[i] = obs_buffer[i].view(1, -1)
                
            # 打印每个观测分量的形状以便调试
            print(f"Observation component {i} shape: {obs_buffer[i].shape}")
